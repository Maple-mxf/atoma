...
public class SemaphoreReleaseCommandHandler
    extends MongoCommandHandler<SemaphoreCommand.Release, Void> {
  ...
  public Void execute(SemaphoreCommand.Release command, MongoCommandHandlerContext context) {
    ...
    Result<Void> result = this.newExecution(client).retryOnCode(WRITE_CONFLICT).execute(cmdBlock);
